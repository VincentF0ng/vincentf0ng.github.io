$(function () {
  ...
  if (highlight_copy == 'true') {
    ...
    var $copyIcon = $('<i class="fas fa-paste" aria-hidden="true"></i>')
    ...
    $('.code-area-wrap .fa-paste').on('click', function () {
  ...
  $(window).scroll(throttle(function (event) {
    ...
    if (currentTop > 56) {
      if (isUp) {
        ...
        if ($('#goback').css('display') === 'block') {
          $('#goback').animate({}, function () {
            $('#goback').css({'opacity': '0','transform': 'translateX(-40px)'})
          })
        }
      } else {
        ...
        if ($('#goback').css('display') === 'block') {
          $('#goback').animate({}, function () {
            $('#goback').css({'opacity': '1','transform': 'translateX(0)'})
          })
        }
      }
  ...
  if (window.location.hash && (window.location.hash.indexOf('%') == -1)) {
    var checkExist = setInterval(function() {
      if ((window.location.hash.indexOf('%') == -1) && $(window.location.hash).length) {
        $("html, body").animate({ scrollTop: $(window.location.hash).offset().top - 90 },500);
